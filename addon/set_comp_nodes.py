...
class LGM_OT_set_lightgroup_postprocess_nodes(bpy.types.Operator):
    ...
    def poll(cls, context):
        return (
            context.scene.use_nodes
            and context.scene.render.engine == "CYCLES"
            and context.scene.node_tree.nodes.active.bl_idname
    ...
    def execute(self, context):
        ...
        lg_list = [
            lightgroup_item.name
            for lightgroup_item in context.scene.view_layers[viewlayer_name].lightgroups
        ]
        ...
        group_node.location = (
            viewlayer_node.location[0] + 300,
            viewlayer_node.location[1],
        )
        group_nt = bpy.data.node_groups.new(
            name="LG_" + viewlayer_name, type="CompositorNodeTree"
        )
        ...
        for i in range(len(lg_list)):
            add_node = self.create_lightgroup_pass_nodes(
                group_nt,
                location=(
                    viewlayer_node.location[0] + (i + 2) * 175,
                    viewlayer_node.location[1] - (i + 1) * 100,
                ),
            )
        ...
        for i, add_node in enumerate(new_nodes):
            if last_add_node is None:
                add_node.inputs[1].default_value = (
                    0,
                    0,
                    0,
                    1,
...
def menu_fun(self, context):
    if (
        ...
        and context.space_data.node_tree.nodes.active.bl_idname
        == "CompositorNodeRLayers"
    ):
